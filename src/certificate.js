...
Certificate.prototype = {
  ...
  click_gen_cert: function ()
  {
    ...
    DOM.qSel('#gen-cert-dlg #c_dav_uid')
     .onchange = (e) =>
     {
       ...
       var up = new Uploader_OPL_WebDav(uid,'')
     ...
     .onchange = (e) =>
       {
         ...
         else if (sel === 'opl_dav' || sel === 'opl_ldp') 
         {
     ...
     .onclick = async () =>
       {
         ...
         if (gen.idp === 'opl_dav') {
           ...
           var up = new Uploader_OPL_WebDav(gen.uid, gen.pwd, gen.dav_path);
         ...
         else if (gen.idp === 'ldp_tls') {
         }
         else if (gen.idp === 'opl_ldp') {
           gen.uid = DOM.qSel('#gen-cert-dlg #c_dav_uid').value;
           gen.pwd = DOM.qSel('#gen-cert-dlg #c_dav_pwd').value;
           gen.cert_dir = DOM.qSel('#gen-cert-dlg #c_cert_path').value;
           gen.dav_path = DOM.qSel('#gen-cert-dlg #c_dav_path').value;
           ...
           var up = new Uploader_OPL_LDP(gen.uid, gen.pwd, gen.dav_path);
           var rc = await up.checkCredentials();
           if (!rc) {
             alert('Wrong DAV User or Pwd');
             return
  ...
  uploadCert: async function(gen, webid, certData)
  {
    ...
    try {
      ...
      {
        var url = new URL(webid);
        var up = new Uploader_LDP_TLS(url.origin + '/');
...
class Uploader_OPL_WebDav extends Uploader {
  ...
  constructor(uid, pwd, base_path)
  {
    ...
    if (base_path) {
      ...
      this.base_path = 'http://id.myopenlink.net/DAV/home/';
      if (uid)
        this.base_path += uid + '/';
    }
...
/***  
  ...
  async uploadFile_0(dir, fname, data, type)
  {
    var path = dir + '/'+ fname;
    var url = this.root+encodeURI(path);
    ...
    var options = {
        ...
        headers: {
          'Authorization': this.createBasicDigest(this.uid, this.pwd),
          'Content-Type': type,
          'Link': this.LDP_RESOURCE + ' <http://www.w3.org/ns/ldp#BasicContainer>; rel="type '
        },
        credentials: 'include',
        body: data
      }
  ...
  async uploadFile_1(dir, fname, data, type)
  {
    var path = dir;
    var url = this.root+encodeURI(path);
