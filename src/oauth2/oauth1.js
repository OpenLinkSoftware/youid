 ...
 */
OAuth1.adapters = {};
OAuth1.adapterReverse = localStorage.oauth1_adapterReverse &&
    JSON.parse(localStorage.oauth1_adapterReverse) || {};
// Update the persisted adapterReverse in localStorage.
if (localStorage.adapterReverse) {
  OAuth1.adapterReverse = JSON.parse(localStorage.adapterReverse);
...
OAuth1.prototype.updateLocalStorage = function() {
  ...
  var data = {};
  var variables = [
    'accessToken', 'accessTokenDate', 'clientId', 'clientSecret',
...
OAuth2.prototype.refreshAccessToken = function(refreshToken, callback) {
  ...
  var formData = "";;
  formData += 'client_id=' + encodeURIComponent(data.clientId) + '&';
  formData += 'client_secret=' + encodeURIComponent(data.clientSecret) + '&';
...
OAuth2.prototype.finishAuth = function() {
  ...
  if (config.mode2) {
    try {
      var access_token = that.adapter.parseAuthorizationAccessToken(window.location.href);
...
OAuth2.prototype.userInfo = function(callback) {
  var that = this;
  OAuth2.loadAdapter(that.adapterName, function() {
    that.adapter = OAuth2.adapters[that.adapterName];
    var data = that.get();
